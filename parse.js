...
module.exports = {
	...
	organizeWeapons: function (json) {
		...
		var gametypes = ["SniperRifle","Rifle","Submachine Gun","Pistol","Knife","Grenade","C4","Shotgun","Machine Gun"];

		// gametype {string} player.weapons.weapon_n.type from game response (example SniperRifle)
		// name     {string} the name of the weapon, or player.weapons.weapon_n.name (example weapon_awp)
		var fixType = function (gameType, name) {
			switch (gameType) {
				// uses fall through property of case-switch statements
				...
				default:
					// for some reason, the zeus has no type, I think it should be "other"
					// however, it's the only weapon with no type, so it still works
					return "Zeus";
			}
		};
		...
		if (csgoJson.player) {
			...
			for (i=0;i<Object.keys(csgoWeapons).length;i++) {
				...
				if (thisWeapon.state == "active") {
					newWeapons.currentWeapon.name = thisWeapon.name;
					// newWeapons.currentWeapon.type = thisWeapon.type;
					newWeapons.currentWeapon.type = fixType(thisWeapon.type,thisWeapon.name);
